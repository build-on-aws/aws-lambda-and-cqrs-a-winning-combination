...
import { CommandsDispatcher } from "../src/dispatchers/CommandsDispatcher";
import { extractCommandsDispatcherContext } from "library-system-common/common/lambda-adapter";
import { QueriesDispatcher } from "../src/dispatchers/QueriesDispatcher";
import { extractQueriesDispatcherContext } from "library-system-common/common/lambda-adapter";
...
describe("Happy path tests for `entrypoint.main`", function () {
  it("Query: GetBooksByAuthor", async () => {
    ...
    const result = (await QueriesDispatcher.create(context, databaseProvider).dispatch()) as Book[];
  ...
  it("Command: AddNewBook", async () => {
    ...
    const databaseProvider = new EmptyDatabaseProvider();
    const context = extractCommandsDispatcherContext(event, lambdaContext);
    const result = (await CommandsDispatcher.create(context, databaseProvider).dispatch()) as AddNewBookCommandResponse;
  ...
  it("Command: BorrowBook", async () => {
    ...
    const databaseProvider = new EmptyDatabaseProvider();
    const context = extractCommandsDispatcherContext(event, lambdaContext);
    const result = (await CommandsDispatcher.create(context, databaseProvider).dispatch()) as BorrowBookCommandResponse;
  ...
  it("Command: ReportMissingBook", async () => {
    ...
    const databaseProvider = new EmptyDatabaseProvider();
    const context = extractCommandsDispatcherContext(event, lambdaContext);
    const result = (await CommandsDispatcher.create(
      context,
      databaseProvider,
    ).dispatch()) as ReportMissingBookCommandResponse;
