...
import { extractCommandsDispatcherContext, extractDatabaseDetails } from "library-system-common/common/lambda-adapter";
...
import { CommandsDispatcher } from "./dispatchers/CommandsDispatcher";
...
const plainHandler = async (event: APIGatewayProxyEvent, lambdaContext: Context): Promise<APIGatewayProxyResult> => {
  try {
    ...
    const response = await CommandsDispatcher.create(context, databaseProvider).dispatch();
